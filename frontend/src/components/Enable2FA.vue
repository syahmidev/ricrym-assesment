<template>
    <div class="bg-gray-100 min-h-screen flex flex-col justify-center items-center pt-6 sm:pt-0">
        <div class="w-full sm:max-w-md px-6 overflow-hidden flex flex-col justify-center items-center" @submit.prevent="login">
            ...
            <input class="mt-6 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block w-full px-3 py-2" v-model="code" placeholder="Enter 2FA Code" />
            <button
                @click="verify2FA"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async verify2FA() {
                try {
                    const response = await axios.post('/auth/verify-2fa', { username: this.getCurrentUsername(), code: this.code });
                    this.$router.push('/');
                } catch (error) {
